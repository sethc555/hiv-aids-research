...
never backfires — robust across global parameter sampling and all three immune models, and
...
treatment is lifelong and the virus rebounds on interruption. The leading cure paradigm is durable
ART-free remission via immune (notably CD8 / "vaccinal-effect") control of the reservoir, supported
by long-acting broadly-neutralizing-antibody trials (e.g. RIO) and combination-immunotherapy results.
...
SHIV viremia >1000-fold in non-human primates (Pitchai et al., *Science* 2024), and that study noted
the TIP "suppressed HIV rebound after interruption of ART" and that "TIP-treated animals exhibited
significantly improved immune responses" — an empirical hint of TIP–immune interplay that has not
been modeled.
...
memory (burden-driven degradation). The rebound clock and PTC fractions are calibrated to clinical
data (untreated/placebo ATI median ~16–22 d; ACTG rebound-by-week-4/12; CHAMP spontaneous PTC ~4–13%;
RIO bNAb durable control ~24%). Parameters and equations are documented in `analysis/METHODS.md`; all
...
**3.2 The benefit is robust across immune structure and never backfires.** The result holds across
all three immune models. Under a wear-down-able memory, a coupled TIP still helped (7/9 regimes) and
never harmed. Under active, burden-driven immune exhaustion combined with a stealthy (low-visibility)
TIP — the configuration most likely to backfire — there was still no backfire: the TIP helped *most*
there (e.g. 17% → ~65% control), because by suppressing the rebound it keeps the antigen burden low
and **spares the immune response from exhaustion**.
...
the nonlinear TIP interference is the natural next step). The conclusions are conditional on the
